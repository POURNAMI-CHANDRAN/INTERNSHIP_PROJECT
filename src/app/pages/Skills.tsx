    ...
    <div className="max-w-5xl mx-auto px-6 py-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 mb-10">
        ...
        <div className="relative group w-full sm:w-[300px]">
          <Search
            size={16}
            className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-indigo-500"
          />
          <input
            type="text"
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-50 transition-all text-sm"
          />
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredSkills.map((skill) => {
              ...
              return (
                <div
                  key={skill._id}
                  className="bg-white p-5 rounded-xl border border-slate-200 hover:border-indigo-200 transition-all flex flex-col justify-between group"
                >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-slate-900/20 backdrop-blur-[2px]">
          <div className="bg-white w-full max-w-sm rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-[11px] font-bold text-slate-400 uppercase mb-1.5">Skill Name</label>
                <input
                  autoFocus
                  value={newSkill.name}
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg outline-none appearance-none text-sm font-medium"
                  >
                    {["General", "Frontend", "Backend", "Database", "DevOps", "Cloud", "Tools", "Tech"].map(cat => (
                      <option key={cat} value={cat}>{cat}</option>
                    ))}
                  </select>
                  <ChevronRight size={14} className="absolute right-3 top-1/2 -translate-y-1/2 rotate-90 text-slate-400" />
                </div>
              </div>
              ...
              <button
                ...
                onClick={handleAdd}
                className="w-full py-2.5 bg-indigo-600 text-white text-sm font-bold rounded-lg hover:bg-indigo-700 disabled:opacity-50 shadow-md shadow-indigo-100 transition-all"
              >
