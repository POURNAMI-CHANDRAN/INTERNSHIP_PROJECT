  ...
  return (
    ...
    >
      ...
      <div className="sticky left-0 z-10 flex items-center gap-3 border-r border-gray-100 bg-white p-4 group-hover:bg-[#FDFDFE]">
        <div className="h-9 w-9 overflow-hidden rounded-full border border-gray-200 bg-gradient-to-br from-sky-100 to-sky-100 flex items-center justify-center shrink-0">
        ...
        <div className="min-w-0">
          <div className="text-md font-bold tracking-tight text-sky-800">
            {employee.name}
          </div>

          <div className="text-[11px] font-medium uppercase tracking-wider text-gray-500">
            {employee.roleId?.name || "Resource"}
          </div>
        </div>
      </div>
      ...
      {monthlyData.map((m) => (
        ...
        >
          <div
            className={`min-h-[150px] w-full flex flex-col rounded-xl transition-all duration-300 border
            ${
              m.hasData
          ...
          >
            ...
            {m.hasData && (
              <div
                className={`flex items-center justify-between px-3 py-2 rounded-t-xl border-b border-black/[0.03] ${m.color} bg-opacity-30`}
              >
                <div className="flex flex-col">
                  <span className="text-[11px] font-bold text-gray-900 tracking-tight leading-none">
                    {(m.totalHours / 160).toFixed(2)}{" "}
            ...
            <div className="flex-1 px-1 py-1">
              {!m.hasData ? (
                <button
                  ...
                  className="flex h-full min-h-[140px] w-full items-center justify-center opacity-0 transition-opacity group-hover/cell:opacity-100"
                >
                  <span className="text-[14px] font-bold text-blue-600 hover:text-blue-700">
                    + Assign
                  </span>
                </button>
              ) : (
                <div className="space-y-1">
                  {m.projects.map((p, idx) => (
                    <div
                      key={idx}
                      onClick={() => handleAssign(m.month, m)}
                      className="px-2 py-2 rounded-lg hover:bg-sky-50/80 transition-colors group/row cursor-pointer"
                    >
                      {/* Project Name & Status */}
