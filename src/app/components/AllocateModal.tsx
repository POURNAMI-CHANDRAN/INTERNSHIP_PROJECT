      ...
      if (mode === "create") {
        ...
        const existingHours =
          employee?.allocations?.reduce(
            (sum: number, a: any) => sum + Number(a?.allocatedHours || 0), 0) || 0;

        const totalAfterAllocation = existingHours + hours;

        if (totalAfterAllocation > MONTHLY_CAPACITY) {
          throw new Error(
            `Employee Exceeds Monthly Capacity (${totalAfterAllocation}/${MONTHLY_CAPACITY}h)`
          );
        }
